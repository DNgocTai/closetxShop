...
export class AllProductsComponent implements OnInit {
  ...
  addProductToCart(item: any) {
    ...
    this._toast.success('Sản phẩm đã được thêm vào giỏ hàng thành công', {
      position: 'top-left',
    });
  }
  ...
  addProductToWishList(item: any, event: any) {
    ...
    if (event.currentTarget.classList.contains('is-favourite')) {
      ...
      this._toast.error('Sản phẩm đã bị xóa khỏi danh sách yêu thích', {
        position: 'top-left',
      });
    } else {
  ...
  productInWishList(itm: any) {
    const cartItemExist = this.WishItems.find(
      (item) => item.product.id === itm.id
    );
  ...
  onScroll() {
    ...
    this.limit =
      this.limit + 20 == 178 || this.limit + 20 > 178 ? 178 : this.limit + 20;
    if (this.limit !== 178)
