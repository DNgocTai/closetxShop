...
export class AuthService {
  ...
  login(email: string, password: string): Observable<any> {
    return this.http.post<any>(`${environment.api}v1/auth/login`, {
      email,
      password,
    });
  }
  ...
  register(name: string, email: string, password: string): Observable<any> {
    ...
    return this.http.post<any>(`${environment.api}v1/users/`, {
      name,
      email,
      password,
      avatar,
  ...
  refreshToken(): Observable<any> {
    ...
    return this.http
      .post<any>(`${environment.api}v1/auth/refresh-token`, { token })
      .pipe(
        map((response: any) => {
